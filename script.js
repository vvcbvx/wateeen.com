...
function truncate(str, maxLength) {
    ...
    try {
        ...
        let currentContent = currentData.record || {};
        ...
        showNotification("حدث خطأ أثناء الحفظ على jsonbin.io", "error");
    } finally {
 ...
 */
// معرف الـ bin الخاص بك على jsonbin.io
const JSONBIN_BIN_ID = '68515de18960c979a5ab639c'; // مثال: 6612b5e31a7e5b66253b6d9c
const JSONBIN_API_KEY = '$2a$10$DY6Bo8O3ePuyfWxRh63SFenGPrD/TXQru7k.//.isDVbR6giTbPKG'; // مثال: $2b$10$xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

/**
 * جلب سعر الصرف من jsonbin.io
 * @returns {Pro
/**
 * جلب سعر الصرف من jsonbin.io
 * @returns {Promise<boolean>} يعيد true إذا نجح الجلب، false إذا فشل
...
async function fetchExchangeRate() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`خطأ في الشبكة: ${response.status}`); // خطأ في الاتصال أو الرابط أو المفتاح
    }
    
    const json = await response.json();
    const content = json.record; // البيانات الحقيقية تكون داخل record
    
    if (!content || typeof content.exchangeRate === 'undefined') {
      throw new Error('سعر الصرف غير موجود في البيانات'); // bin لا تحتوي exchangeRate
    }
    ...
    // إذا لم ينجح الجلب من jsonbin.io لأي سبب
    console.error("فشل في جلب البيانات:", error);
    
    // محاولة استرجاع آخر سعر صرف مخزن محلياً
